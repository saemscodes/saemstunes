...
const EnhancedAnimatedList: React.FC<EnhancedAnimatedListProps> = ({ tracks, className, onTrackSelect }) => {
  ...
  const { user } = useAuth();
  const { state: audioState, playTrack } = useAudioPlayer();
  const { queue, addItemToQueue, playPlaylist } = usePlaylist();
  ...
  const currentPlaylist = queue.currentPlaylist;
  ...
  const handlePlayTrack = (track: AudioTrack) => {
    // First call the onTrackSelect callback if provided
    if (onTrackSelect) {
      onTrackSelect(track);
    } else {
    ...
    // Create playable item for audio player
    const playableItem = {
      id: track.id.toString(),
      type: 'track' as const,
      title: track.name,
  ...
  const isTrackPlaying = (track: AudioTrack) => {
    return currentTrack?.id === track.id.toString();
  };
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="relative h-12 w-12 flex-shrink-0">
              ...
              <div className={cn(
                ...
                isTrackPlaying(track) ? "opacity-100" : "opacity-0 group-hover:opacity-100"
              )}>
              ...
              {/* Queue indicator */}
              {isTrackInQueue(track) && !isTrackPlaying(track) && (
